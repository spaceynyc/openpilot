...
DriveStats::DriveStats(QWidget *parent) : QFrame(parent) {
  ...
  addStatsLayouts(tr("STARPILOT"), frogPilot, true);
  ...
  setStyleSheet(R"(
    ...
    QLabel[type="starpilot_title"] { font-size: 50px; font-weight: 500; color: #178643; }
...
void DriveStats::addStatsLayouts(const QString &title, StatsLabels &labels, bool StarPilot) {
  QGridLayout *grid_layout = new QGridLayout;
  grid_layout->setVerticalSpacing(10);
  grid_layout->setContentsMargins(0, 10, 0, 10);

  int row = 0;
  grid_layout->addWidget(newLabel(title, StarPilot ? "starpilot_title" : "title"), row++, 0, 1, 3);
...
void DriveStats::updateStarPilotStatsForLabel(StatsLabels &labels) {
  QJsonObject starpilot_stats = QJsonDocument::fromJson(QByteArray::fromStdString(params.get("StarPilotStats"))).object();

  labels.distance->setText(QString::number(int(starpilot_stats.value("StarPilotMeters").toDouble() * (isMetric ? 0.001 : METER_TO_MILE))));
  labels.distance_unit->setText(isMetric ? tr("KM") : tr("Miles"));
  labels.hours->setText(QString::number(int(starpilot_stats.value("StarPilotSeconds").toDouble() / (60 * 60))));
  labels.routes->setText(QString::number(starpilot_stats.value("StarPilotDrives").toInt()));
}
...
void DriveStats::updateStats() {
  ...
  updateStarPilotStatsForLabel(frogPilot);
