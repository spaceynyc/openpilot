...
StarPilotDriveSummary::StarPilotDriveSummary(QWidget *parent, bool randomEvents) : QFrame(parent), displayRandomEvents(randomEvents) {
  ...
  QObject::connect(uiState(), &UIState::offroadTransition, [this](bool offroad) {
    if (!offroad) {
      previousStats = QJsonDocument::fromJson(QString::fromStdString(params.get("StarPilotStats")).toUtf8()).object();
    }
  });
}

void StarPilotDriveSummary::mousePressEvent(QMouseEvent *e) {
  emit panelClosed();
}

void StarPilotDriveSummary::showEvent(QShowEvent *event) {
  bool isMetric = params.getBool("IsMetric");

  QJsonObject currentStats = QJsonDocument::fromJson(QString::fromStdString(params.get("StarPilotStats")).toUtf8()).object();
  ...
  if (displayRandomEvents) {
    ...
    frogPilotMetersValue->setText(formatDistance(diffDouble("StarPilotMeters")));
    trackedTimeValue->setText(formatTime(trackedTime));
  }
}

void StarPilotDriveSummary::hideEvent(QHideEvent *event) {
  emit panelClosed();
}

QWidget *StarPilotDriveSummary::createStatBox(const QString &title, QLabel **valueLabel, QWidget *parent) {
